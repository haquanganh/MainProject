...
@section('name','Edit Client Company')
@section('css')
<link rel="stylesheet" type="text/css" href="{{ asset('css/admin/edit_personal.css') }}">
@stop
@section('content')
<form method="POST" action="{{ url('admin/personal-information/clientcompany/edit') }}/{{ $clientcompany->idClientCompany }}" enctype="multipart/form-data">
{{ csrf_field() }}
<div class="col-md-7 right-regis">
		<div class="row form-group {{ $errors->has('in_Name') ? ' has-error' : '' }}">
			...
			<div class="col-md-8">
				<input name="in_Name" onkeypress="validate_spec4(event)" class="form-control" value="{{isset($clientcompany->CC_Name) && $errors->has('in_Name') == false && old('in_Name') == '' ? $clientcompany->CC_Name : old('in_Name')}}">
			</div>
		...
		<div class="row form-group {{ $errors->has('in_Phone') ? ' has-error' : $errors->has('wrong_phone') ? 'has-error' : '' }}">
			...
			<div class="col-md-8">
				<input name="in_Phone" onkeypress="validate(event)" type="text" class="form-control" value="{{isset($clientcompany->CC_Phone) && $errors->has('in_Phone') == false && $errors->has('wrong_phone') == false && old('in_Phone') == '' ? '0'.$clientcompany->CC_Phone : old('in_Phone')}}">
			</div>
		...
		<div class="row form-group {{ $errors->has('in_Skype') ||$errors->has('wrong_skype') ? ' has-error' : '' }}">
			...
			<div class="col-md-8">
				<input name="in_Skype" onkeypress="validate_spec(event)"  maxlength="32" type="text" class="form-control" value="{{isset($clientcompany->CC_Skype) && $errors->has('in_Skype') == false && $errors->has('wrong_skype') == false && old('in_Skype') == '' ? $clientcompany->CC_Skype : old('in_Skype')}}">
			</div>
		...
		<div class="row form-group {{ $errors->has('in_Address')  ? ' has-error' : $errors->has('wrong_address') ? 'has-error': ''  }}">
			...
			<div class="col-md-8">
				<input name="in_Address" onkeypress="validate_spec5(event)" type="text" class="form-control" value="{{isset($clientcompany->CC_Address) && $errors->has('in_Address') == false && $errors->has('wrong_address') == false && old('in_Address') == '' ? $clientcompany->CC_Address : old('in_Address')}}">
			</div>
	...
	<div class="col-md-5 left-regis">
		<div class="avatar-img">
			<img src="{{($clientcompany->CC_Logo != NULL && File::exists(public_path('images/personal_images/'.$clientcompany->C_Logo)) ) ? asset('images/personal_images/'.$clientcompany->CC_Logo): asset('images/notfound.jpg')}}" style="margin:0px;">
