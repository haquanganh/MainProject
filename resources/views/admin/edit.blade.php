...
<div class="col-md-7 right-regis">
    ...
    <?php
        $project = DB::select('select * from Employee inner join ProjectEmployee on Employee.idEmployee = ProjectEmployee.idEmployee inner join Project on ProjectEmployee.idProject = Project.idProject where idPStatus = 1 and ProjectEmployee.idEmployee = '.$employee->idEmployee);

    ?>
    <input type="hidden" id="check-project" name="in_checkproject" value="{{ !empty($project) ? 'NOT OK' : 'OK' }}">
	...
	<div class="row form-group {{ $errors->has('sl_Team') ? ' has-error' : '' }}">
		...
		<div class="col-md-8">
			<select id="Team_select" name="sl_Team" class="form-control">
		...
		<tbody class="content-skill text-center">
			@foreach ($employee->Skill()->get() as $k=>$e)
				<tr>
                    ...
                    <td><input type="text" onkeypress="validate(event)" name="in_Level[{{$k}}]" value="{{$e->pivot->S_Level}}"></td>
					<td><a class="aDel"><i class="fa fa-times" aria-hidden="true"></i></a></td>
				</tr>
			@endforeach
		</tbody>
		</table>
...
function myFunction() {
    ...
    cell1.innerHTML = '<input type="number" onkeypress="validate(event)" min="1" max="100" name="in_Year['+(t-2)+']">';
    ...
    cell3.innerHTML = '<input type="number" onkeypress="validate(event)" min="1" max="10" name="in_Level['+(t-2)+']">';
	cell4.innerHTML = '<a class="aDel"><i class="fa fa-times" aria-hidden="true"></i></a>';
...
<script>
    jQuery(document).ready(function($) {
        if($('#check-project').val() == 'NOT OK'){
            $('#Team_select').attr('disabled', 'true');
        }
    });
</script>
@stop