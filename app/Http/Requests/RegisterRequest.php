...
class RegisterRequest extends Request
{
    ...
    public function rules()
    {
        ...
        if($this->request->get('sl_Role') == true){
            if($this->request->get('sl_Role') == "Manager" || $this->request->get('sl_Role') == "Member"){
                ...
                if($this->request->get('in_Level') == true){
                    foreach ($this->request->get('in_Level') as $key => $value) {
                    $rules['in_Level.'.$key] = 'required';
    ...
    public function messages(){
        $messages= [
            ...
            'in_Level.0.required' => 'Please add level',
        ...
        if($this->request->get('sl_Role') == true){
            if($this->request->get('sl_Role') == "Manager" || $this->request->get('sl_Role') == "Member" ){
                ...
                if($this->request->get('in_Level') == true){
                    foreach ($this->request->get('in_Level') as $key => $value) {
                        $messages['in_Level.'.$key.'.required'] = 'Please enter the level';
                    }
                }
