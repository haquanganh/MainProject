...
class EmployeeController extends Controller
{
    public function getEmployee(){
        ...
        if($idRole == 6)
        {
            $listE = DB::select( DB::raw("SELECT Employee.*, Role.Role FROM Employee JOIN users ON Employee.idAccount = users.idAccount JOIN Role ON users.idRole = Role.idRole"));
            $kq = array();
            for ($i=0; $i < sizeof($listE); $i++) {
                ...
                return view('personal.employee_information',compact('kq', 'idRole'));
        }
        else
        {
            if($idRole == 4)
    ...
    public function postEmployee(Request $request){
        ...
        if($idRole == 6)
        {
            ...
            {
                $listE = DB::select( DB::raw("SELECT Employee.*, Role.Role FROM Employee JOIN   users ON Employee.idAccount = users.idAccount JOIN Role ON users.idRole = Role.idRole WHERE Employee.E_Name LIKE '%$search%'"));
            }
            else if($search_type == 'Search by skill')
            {
                $listE = DB::select( DB::raw("SELECT Employee.*, Role.Role FROM Employee JOIN SkillDetail ON Employee.idEmployee = SkillDetail.idEmployee JOIN Skill ON SkillDetail.idSkill = Skill.idSkill JOIN users ON Employee.idAccount = users.idAccount JOIN Role ON users.idRole = Role.idRole WHERE Skill.Skill LIKE '%$search%'"));
            }
            else if($search_type == 'Search by cost/hour')
            {
                $listE = DB::select( DB::raw("SELECT Employee.*, Role.Role FROM Employee JOIN users ON Employee.idAccount = users.idAccount JOIN Role ON users.idRole = Role.idRole WHERE Employee.E_Cost_Hour = '$search'"));
            }
            if(count($listE) != 0 && $search != NULL)
            {
                ...
                return view('personal.employee_information', compact('list_search', 'idRole'))
